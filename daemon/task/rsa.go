...
func setPublicKey() {
	...
	url := common.Proto + "://" + common.WebIP + common.Puplic_Key_API
	resp, err := common.HTTPClient.Get(url)
	if err != nil {
		log.Debug("HTTP get publickey error:%v", err.Error())
		return
	}
	defer resp.Body.Close()
	result, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		log.Debug("Update publickey ioutil.ReadAll error:%v", err.Error())
		return
	}
